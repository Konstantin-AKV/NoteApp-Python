...
class Buttons:
    def __init__(self, frame, table):
        ...
        self.creation_button = ttk.Button(frame, text="Создать новую заметку", command=self.edit)
        self.creation_button.pack()

        self.edit_button = ttk.Button(frame, text="Открыть/изменить", state=DISABLED,
                                      command=lambda: self.edit(update=True))
        self.edit_button.pack()

        self.delete_button = ttk.Button(frame, text="Удалить заметку", state=DISABLED,
                                        command=lambda: [
        ...
        self.refresh_button = ttk.Button(frame, text="Обновить таблицу", command=table.refresh)
